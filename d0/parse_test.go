...
func TestParsing(t *testing.T) {
	...
	result, err := p.GetOBISMap(d0.Mock_ebz1str, mt)
	assert.NoError(t, err)
	assert.Equal(t, "EBZ5DD12345ETA_104", result.DeviceID)
	assert.Len(t, result.List, 5)

	result, err = p.GetOBISMap(d0.Mock_ebz2str, mt)
	assert.NoError(t, err)
	assert.Equal(t, "EBZ5DD12345ETA_104", result.DeviceID)
	assert.Len(t, result.List, 9)

	result, err = p.GetOBISMap(d0.Mock_ebz3str, mt)
	assert.NoError(t, err)
	assert.Equal(t, "EBZ5DD12345ETA_104", result.DeviceID)
	assert.Len(t, result.List, 9)

	result, err = p.GetOBISMap(d0.Mock_lugcuh50_1, mt)
	assert.NoError(t, err)
	assert.Equal(t, "LUGCUH50", result.DeviceID)
	assert.Len(t, result.List, 65)

	result, err = p.GetOBISMap(d0.Mock_lugcuh50_2, mt)
	assert.NoError(t, err)
	assert.Equal(t, "LUGCUH50", result.DeviceID)
	assert.Len(t, result.List, 65)
